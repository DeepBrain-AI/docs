...
4. **Idle 중 Keep-Alive**: `POST /api/v2/sessions/{sessionId}/keep-alive`
   - 사용자가 idle 상태일 때 session activity 갱신
   - `next_ping_recommended_sec`초 간격으로 호출 (기본값: 120)
5. **Talk**: `POST /api/v2/sessions/{sessionId}/talk`
   - 사용자 메시지 전송
   - 아바타가 메시지를 발화
   - greeting_text를 설정했으면 자동으로 첫 메시지 전송
6. **Stop Session**: `POST /api/v2/sessions/stop`
...
| 4020 | 402       | 과금 / 인터랙티브 크레딧 부족 |
| 4030 | 403       | 권한 없음                     |
| 4040 | 404       | 리소스 없음                   |
| 4090 | 409       | 충돌, 예: 이미 종료된 session |
| 4290 | 429       | 레이트 리밋                   |
| 5000 | 500       | 내부/서비스 오류              |
