...
All API communications within AI STUDIO V3 require authentication. The API key is used for this purpose. Set the issued API key in the `token` variable. If you do not have an issued key yet, you can issue it at [Generate API Key](../generate-api-key).
...
Set the `projectId` variable to the ID of an existing project. You can find the project ID by clicking on a previously saved project on the [My Studio](https://app.aistudios.com/auth/signin) page and entering the edit screen via the URL. For example, in `app.aistudios.com/editor/abcdefg`, the project ID is `abcdefg`.
