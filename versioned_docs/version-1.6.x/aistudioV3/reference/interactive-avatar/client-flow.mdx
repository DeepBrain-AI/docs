...
4. **Keep-Alive while idle**: `POST /api/v2/sessions/{sessionId}/keep-alive`
   - Refresh session activity when the user is idle
   - Call about every `next_ping_recommended_sec` seconds (default: 120)
...
| 4020 | 402       | Billing / interactive credit insufficient |
| 4030 | 403       | Forbidden                                 |
| 4040 | 404       | Resource not found                        |
| 4090 | 409       | Conflict, e.g. session already ended      |
